...
//        Maksimalno trajanje poziva je (broj minuta)min
        ...
        host.setIme("Pera");
        host.setPrezime("Peric");

        Korisnik guest = new Korisnik();
        guest.setIme("Mika");
        guest.setPrezime("Mikic");

        ZoomCall call = new ZoomCall("link.sda", host, "pass123");
        call.setGuest(guest);
        call.pokreniPoziv();

        host.pretplatiSe(100);

        ZoomCall call2 = new ZoomCall("link2.da", host, "pass321");
        call2.setGuest(guest);
        call2.pokreniPoziv();
